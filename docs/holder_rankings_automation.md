...
- Reported logs must match canonical block hashes. Indexed transfers are compared with the exact multiset of hash-bound token logs for each reported block before balances change. Verified Alchemy windows are capped at 20,000 blocks, including when a larger window is configured; smaller windows remain supported.
- Balance reconciliation and its individual fallback use the actual saved checkpoint hash with `requireCanonical`, including after partial progress. Verification caches last only for their scan phase. Entirely omitted blocks still depend on provider completeness; the detailed plan describes that limit and the added RPC reads.
