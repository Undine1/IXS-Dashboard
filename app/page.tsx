...
export default function Dashboard() {
  ...
  // A pool that failed its RPC read arrives with value null — that is a
  // degraded state even when the route reported no explicit warnings.
  const poolsHaveGaps = pools.some((pool) => typeof pool.value !== 'number' || !Number.isFinite(pool.value));

  const poolIndexerStatus = loading
    ? 'Syncing'
    : pools.length === 0
      ? 'Offline'
      : warnings.length > 0 || poolsHaveGaps
        ? 'Warning'
        : 'Healthy';
        ...
        {loading ? (
          <div className="py-8 md:py-10">
            <div className="mx-auto w-full max-w-2xl">
              ...
              <div className="mt-3 h-4 w-full rounded-md border bg-transparent p-0.5 border-slate-600/90">
                <div
                  className="h-full rounded-sm transition-[width] duration-200 ease-out bg-slate-200"
