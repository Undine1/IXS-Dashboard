...
A production-ready analytics dashboard that tracks IXS token burns and Total Value Locked (TVL) across multiple chains (Ethereum, Polygon, Base). It uses lightweight on-chain reads (JSON-RPC / eth_call and eth_getLogs) and a small set of update scripts to maintain aggregated pool volumes and burn statistics written to `public/data`.
...
- Approach: Prefer direct `eth_getLogs` for accuracy; use an indexer (Etherscan-compatible) as a fallback and to reduce provider strain. Scripts persist results to `public/data/` so the Next.js app can serve stable, cached artifacts.
...
- `scripts/` — updater scripts
  ...
  - `update_pool_volume_rpc.js` — RPC-mode updater using `eth_getLogs` with chunking; indexer fallback available
...
- `POLYGON_RPC` / other RPC endpoints — optional, may be used by RPC-mode updater if present
...
```bash
cd scripts
node update_pool_volume_rpc.js   # RPC-mode scanner (may be slower / require RPC limits)
node update_pool_volume_indexer.js  # indexer-backed incremental updater (recommended for large scans)
```
...
- RPC script uses chunked `eth_getLogs` and a timestamp->block binary search to find blocks; it includes pacing and deterministic per-pool jitter to avoid provider bursts.
- Both scripts maintain per-pool checkpoints in `public/data/pool_volume_checkpoint.json` so they can resume without re-scanning completed ranges.
...
## Troubleshooting
- If a workflow run fails due to `pool_volume_alert.json`, inspect the file to determine which pool or API exhausted retries.
- For RPC `eth_getLogs` issues: verify RPC provider quotas and consider running the indexer-backed updater instead.
