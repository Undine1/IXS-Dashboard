...
- Reported transfer hashes are checked against canonical headers; indexed transfers are matched against hash-bound token logs before counting. Pool anchors pin token units, and holder reconciliation uses the actual checkpoint hash. These checks still rely on providers reporting complete history; the plan documents the limits and additional RPC cost.
